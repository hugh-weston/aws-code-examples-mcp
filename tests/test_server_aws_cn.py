...
import httpx
import pytest
from code_example_server.server_aws_cn import (
...
class TestReadDocumentationChina:
    ...
    async def test_read_documentation_china(self):
        ...
        with patch('httpx.AsyncClient.get', new_callable=AsyncMock) as mock_get:
            ...
            with patch(
                'code_example_server.server_utils.extract_content_from_html'
            ) as mock_extract:
...
class TestGetAvailableServices:
    ...
    async def test_get_available_services(self):
        ...
        with patch('httpx.AsyncClient.get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = mock_response
            with patch(
                'code_example_server.server_aws_cn.extract_content_from_html'
            ) as mock_extract:
    ...
    async def test_get_available_services_non_html(self):
        ...
        with patch('httpx.AsyncClient.get', new_callable=AsyncMock) as mock_get:
            mock_get.return_value = mock_response
            with patch(
                'code_example_server.server_aws_cn.is_html_content'
            ) as mock_is_html:
...
class TestMain:
    ...
    def test_main(self):
        ...
        with patch('code_example_server.server_aws_cn.mcp.run') as mock_run:
            with patch(
                'code_example_server.server_aws_cn.logger.info'
            ) as mock_logger:
                main()
