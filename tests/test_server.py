...
class TestServer:
    ...
    def test_partition_aws(self):
        ...
        with patch.dict(os.environ, {'AWS_DOCUMENTATION_PARTITION': 'aws'}):
            with patch('code_example_server.server_aws.main') as mock_main:
                from code_example_server.server import main
    ...
    def test_partition_aws_cn(self):
        ...
        with patch.dict(os.environ, {'AWS_DOCUMENTATION_PARTITION': 'aws-cn'}):
            with patch('code_example_server.server_aws_cn.main') as mock_main:
                import code_example_server.server

                # Need to reload the module to pick up the environment variable change
                import importlib
                from code_example_server.server import main

                importlib.reload(code_example_server.server)
                main()
                mock_main.assert_called_once()
